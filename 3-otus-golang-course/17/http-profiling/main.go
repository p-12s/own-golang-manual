...
// userSearchRequest - ловим данные от пользователя и конструируем для дальнейш. работы
...
func handleUserSearchWithPool(ctx *fasthttp.RequestCtx) {
	req := requestsPool.Get().(*userSearchRequest) // если пула еще нет - он создастся
	resp := doUserSearch(req, ctx.URI().QueryArgs())
	ctx.Write(resp)
	requestsPool.Put(req) // если не класть обратно в пул - код наоборот будет замедляться
}
